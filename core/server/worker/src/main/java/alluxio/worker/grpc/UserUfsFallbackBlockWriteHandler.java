...
@NotThreadSafe
public final class UserUfsFallbackBlockWriteHandler
  ...
  private final UserBlockWriteHandler mUserBlockWriteHandler;
   ...
   */
  UserUfsFallbackBlockWriteHandler(BlockWorker blockWorker, UfsManager ufsManager,
    ...
    mUserId = userId;
    mUserBlockWriteHandler =
        new UserBlockWriteHandler(blockWorker, responseObserver, userInfo, domainSocketEnabled, userId);
  }
  ...
  protected void completeRequest(BlockWriteRequestContext context)
      throws Exception {
    if (context.isWritingToLocal()) {
      mUserBlockWriteHandler.completeRequest(context);
    } else {
  ...
  protected void cancelRequest(BlockWriteRequestContext context) throws Exception {
    if (context.isWritingToLocal()) {
      mUserBlockWriteHandler.cancelRequest(context);
    } else {
      if (context.getOutputStream() != null) {
        context.getOutputStream().close();
        context.setOutputStream(null);
  ...
  protected void cleanupRequest(BlockWriteRequestContext context) throws Exception {
    if (context.isWritingToLocal()) {
      mUserBlockWriteHandler.cleanupRequest(context);
    } else {
  ...
  protected void flushRequest(BlockWriteRequestContext context) throws Exception {
    if (context.isWritingToLocal()) {
      mUserBlockWriteHandler.flushRequest(context);
  ...
  protected void writeBuf(BlockWriteRequestContext context,
    ...
    if (context.isWritingToLocal()) {
      ...
      try {
        mUserBlockWriteHandler.writeBuf(context, responseObserver, buf, pos);
        return;
