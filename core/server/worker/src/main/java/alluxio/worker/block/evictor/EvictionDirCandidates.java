...
class EvictionDirCandidates {
  ...
  private Map<StorageDirEvictorView, Pair<List<Pair<Long, Long>>, Long>> mDirCandidatesMap = new HashMap<>();
  ...
  public void add(StorageDirEvictorView dir, long userId, long blockId, long blockSizeBytes) {
    Pair<List<Pair<Long, Long>>, Long> candidate;
    if (mDirCandidatesMap.containsKey(dir)) {
      candidate = mDirCandidatesMap.get(dir);
    } else {
      candidate = new Pair<List<Pair<Long, Long>>, Long>(new ArrayList<Pair<Long, Long>>(), 0L);
      mDirCandidatesMap.put(dir, candidate);
    }

    candidate.getFirst().add(new Pair<>(blockId, userId));
    long blockBytes = candidate.getSecond() + blockSizeBytes;
    candidate.setSecond(blockBytes);
  ...
  public List<Pair<Long, Long>> candidateUserBlocks(){
    Pair<List<Pair<Long, Long>>, Long> evict = mDirCandidatesMap.get(mDirWithMaxBytes);
    if (evict == null){
      return new ArrayList<>();
    }
    return evict.getFirst();
