    ...
    if (mBlockWriter != null && mBlockWriter.getPosition() < mInStreamPos) {
      try {
        ...
        if (userId == -1L){
          ...
          mLocalBlockStore.requestUserSpace(mBlockMeta.getSessionId(), userId, mBlockMeta.getBlockId(),
              mInStreamPos - mBlockWriter.getPosition());
        }
      ...
      try {
        ...
        while (bufCopy.readableBytes() > 0) {
          if (mBlockMeta.getUserId() == -1){
            ...
            mLocalBlockStore.requestUserSpace(mBlockMeta.getSessionId(), mBlockMeta.getUserId(), mBlockMeta.getBlockId(),
                mInStreamPos - mBlockWriter.getPosition());
          }
  ...
  private void cancelBlockWriter() throws IOException {
    ...
    try {
      ...
      if (mBlockMeta.getUserId() == -1L){
        ...
        mLocalBlockStore.abortUserBlock(mBlockMeta.getSessionId(), mBlockMeta.getUserId(), mBlockMeta.getBlockId());
